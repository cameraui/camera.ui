...
def _rewrite_url(url: str, master: str, user: str | None, password: str | None) -> str:
    ...
    if user and rewritten.startswith("rtsp://") and "@" not in rewritten[7:].split("/")[0]:
...
def _walk(value: Any, master: str, user: str | None, password: str | None) -> Any:
    ...
    if isinstance(value, list):
        return [_walk(v, master, user, password) for v in value]  # pyright: ignore[reportUnknownVariableType]
    if isinstance(value, dict):
        return {k: _walk(v, master, user, password) for k, v in value.items()}  # pyright: ignore[reportUnknownVariableType]
    return value
