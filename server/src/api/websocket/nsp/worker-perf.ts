...
export function deriveWorkerPerf(snapshot: FrameWorkerPerfSnapshot, pluginName: (pluginId: string) => string | undefined): WorkerPerfStats {
  ...
  return {
    ...
    objectFrames: snapshot.framesWithObjects,
    objectFps: snapshot.objectLoopMs > 0 ? round(snapshot.framesWithObjects / (snapshot.objectLoopMs / 1000)) : 0,
    objectFrameMs: per(snapshot.objectWorkMs, snapshot.framesWithObjects),
    ...
    vectorsDropped: snapshot.vectorsDropped,
    closeDelays: snapshot.closeDelays,
    closeDelayMs: per(snapshot.closeDelayMs, snapshot.closeDelays),
