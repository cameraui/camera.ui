...
export declare type VirtualScrollerPassThroughOptionType =
  | VirtualScrollerPassThroughAttributes
  | ((options: VirtualScrollerPassThroughMethodOptions) => VirtualScrollerPassThroughAttributes | string)
  | string
  | null
  | undefined;
...
/**
 * Defines current inline state in CuiVirtualScroller component.
 */
...
/**
 * Defines valid properties in CuiVirtualScroller component.
 */
export interface VirtualScrollerProps {
  ...
  /**
   * RTL for horizontal orientation.
   * @defaultValue false
   */
  ...
  /**
   * If disabled, the CuiVirtualScroller feature is eliminated and the content is displayed directly.
   * @defaultValue false
   */
  disabled?: boolean | undefined;
  /**
   * Used to implement a custom loader instead of using the loader feature in the CuiVirtualScroller.
   * @defaultValue false
   */
  ...
  /**
   * Used to implement a custom spacer instead of using the spacer feature in the CuiVirtualScroller.
   * @defaultValue true
   */
  ...
  unstyled?: boolean;
  /**
   * Optional key resolver for rendered items. Accepts either a field name
   * (string) or a `(item, index) => string | number` function. When unset,
   * items are keyed by their position in the render window — which causes
   * slot content to remount on every scroll-driven range shift. Set this
   * to something stable (e.g. an id) to keep slot VNodes alive across
   * scroll.
   */
  itemKey?: string | ((item: unknown, index: number) => string | number | undefined) | null;
}

/**
 * Defines valid slots in CuiVirtualScroller component.
 */
...
/**
 * Defines valid emits in CuiVirtualScroller component.
 */
 ...
 *
 */
declare const CuiVirtualScroller: DefineComponent<VirtualScrollerProps, VirtualScrollerSlots, VirtualScrollerEmits, VirtualScrollerMethods>;

declare module 'vue' {
  export interface GlobalComponents {
    CuiVirtualScroller: DefineComponent<VirtualScrollerProps, VirtualScrollerSlots, VirtualScrollerEmits, VirtualScrollerMethods>;
  }
}

export default CuiVirtualScroller;
