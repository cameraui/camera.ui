<template>
  ...
  <template v-if="!disabled">
    <div :ref="elementRef" :class="containerClass" :tabindex="tabindex" :style="style" v-bind="ptmi('root')" @scroll="onScroll">
      <slot
        name="content"
        :style-class="contentClass"
        :items="loadedItems"
        :get-item-options="getOptions"
        :loading="d_loading"
        :get-loader-options="getLoaderOptions"
        :item-size="itemSize"
        :rows="loadedRows"
      ...
      >
        <div :ref="contentRef" :class="contentClass" :style="contentStyle" v-bind="ptm('content')">
          <template v-for="(item, index) of loadedItems" :key="resolveItemKey(item, index)">
...
/**
 *
 * CuiVirtualScroller is a performant approach to handle huge data efficiently.
 *
...
export default {
    ...
    containerClass() {
      return [
        ...
        this.class,
        {
          ...
          'p-virtualscroller-rtl': this.rtl && this.isHorizontal(),
        },
      ];
    },
    contentClass() {
      ...
      if (this.items && !this.d_loading) {
        if (this.isBoth())
          return this.items
            ...
            .map((item) => (this.columns ? item : item.slice(this.appendOnly ? 0 : this.first.cols, this.last.cols)));
  ...
  watch: {
    ...
    // No `deep: true` — the handler only checks length, and Vue 3 array proxies
    // already trigger non-deep watchers on push/splice/etc.
  ...
  methods: {
    ...
    // Coalesce repeated init() invocations within the same tick into a single
    // microtask-tailed run. Watchers on items/itemSize/scrollHeight/scrollWidth
    // can all fire back-to-back during initial list population; each init() does
    // forced-layout reads (offsetWidth/offsetHeight) so running it once per tick
    // instead of once per mutation is a direct cut to the Safari layout storm.
    scheduleInit() {
    ...
    setContentPosition(pos) {
      if (this.content && !this.appendOnly) {
        ...
        const setTransform = (_x = 0, _y = 0) => {
          const translateX = rtl ? -_x : _x;

          this.contentStyle = {
            ...this.contentStyle,
            ...{ transform: `translate3d(${translateX}px, ${_y}px, 0)` },
          };
        };
        ...
        if (both) {
          ...
          setTransform(calculateTranslateVal(firstCols, itemSize[1]), calculateTranslateVal(firstRows, itemSize[0]));
        } else {
          const firstValue = first;
          const itemSize = this.itemSize;
          const translateVal = calculateTranslateVal(firstValue, itemSize);
    ...
    onScrollPositionChange(event) {
      ...
      let scrollLeft = calculateScrollPos(target.scrollLeft, contentPos.left);

      if (rtl) {
        const isNegative = scrollLeft < 0;
        if (isNegative) {
      ...
      if (both) {
        ...
        if (!this.appendOnly || (this.appendOnly && (isScrollDown || isScrollRight))) {
          ...
          const triggerIndex = {
            rows: this.calculateTriggerIndex(currentIndex.rows, this.first.rows, this.last.rows, this.numItemsInViewport.rows, this.d_numToleratedItems[0], isScrollDown),
            cols: this.calculateTriggerIndex(
              currentIndex.cols,
              this.first.cols,
              this.last.cols,
          ...
          newLast = {
            rows: this.calculateLast(currentIndex.rows, newFirst.rows, this.last.rows, this.numItemsInViewport.rows, this.d_numToleratedItems[0]),
            cols: this.calculateLast(currentIndex.cols, newFirst.cols, this.last.cols, this.numItemsInViewport.cols, this.d_numToleratedItems[1], true),
          };
          ...
          newScrollPos = { top: scrollTop, left: target.scrollLeft };
        }
      } else {
        const scrollPos = horizontal ? scrollLeft : scrollTop;
        ...
        if (!this.appendOnly || (this.appendOnly && isScrollDownOrRight)) {
          ...
          const triggerIndex = this.calculateTriggerIndex(currentIndex, this.first, this.last, this.numItemsInViewport, this.d_numToleratedItems, isScrollDownOrRight);

          newFirst = this.calculateFirst(currentIndex, triggerIndex, this.first, this.last, this.numItemsInViewport, this.d_numToleratedItems, isScrollDownOrRight);

          newLast = this.calculateLast(currentIndex, newFirst, this.last, this.numItemsInViewport, this.d_numToleratedItems);
    ...
    calculateTriggerIndex(currentIndex, first, last, num, numT, isScrollDownOrRight) {
      return currentIndex <= numT ? numT : isScrollDownOrRight ? last - num - numT : first + numT - 1;
    },
    calculateFirst(currentIndex, triggerIndex, first, last, num, numT, isScrollDownOrRight, isCols = false) {
      if (currentIndex <= numT) return 0;

      const firstValue = Math.max(
        0,
        isScrollDownOrRight ? (currentIndex < triggerIndex ? first : currentIndex - numT) : currentIndex > triggerIndex ? first : currentIndex - 2 * numT,
      );

      const maxFirst = this.getLast(firstValue, isCols);
      if (firstValue > maxFirst) return maxFirst - num;
      else return firstValue;
    },

    calculateLast(currentIndex, first, last, num, numT, isCols = false) {
      let lastValue = first + num + 2 * numT;

      if (currentIndex >= numT) {
        lastValue += numT + 1;
      }
