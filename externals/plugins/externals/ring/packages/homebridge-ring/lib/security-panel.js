...
export class SecurityPanel extends BaseDeviceAccessory {
    ...
    constructor(device, accessory, config) {
        ...
        const { Characteristic, Service } = hap, validValues = [
    ...
    set targetingNightMode(value) {
        this.accessory.context.targetingNightMode = value;
    }
    async setTargetState(state) {
        const { Characteristic: { SecuritySystemTargetState: State }, } = hap, { location } = this.device, { nightModeBypassFor } = this.config;
        let bypass = false;
        ...
        const bypassContactSensors = bypass
            ? (await location.getDevices()).filter((device) => {
                return (device.deviceType === RingDeviceType.ContactSensor &&
                    device.data.faulted);
            })
