...
export class Chime extends BaseDataAccessory {
    device;
    accessory;
    config;
    constructor(device, accessory, config) {
        ...
        this.config = config;
        const { Characteristic, Service } = hap, snoozeService = this.getService(Service.Switch, device.name + ' Snooze', 'snooze'), playDingService = this.getService(Service.Switch, device.name + ' Play Ding', 'play-ding'), playMotionService = this.getService(Service.Switch, device.name + ' Play Motion', 'play-motion'), hasNightlight = device.data.settings.night_light_settings !== undefined;
        // Snooze Switch
        ...
        if (hasNightlight) {
            const nightlightService = this.getService(Service.Lightbulb, device.name + ' Nightlight', 'nightlight');
            this.registerCharacteristic({
                characteristicType: Characteristic.On,
                serviceType: nightlightService,
                getValue: (data) => Boolean(data.settings.night_light_settings?.light_sensor_enabled),
                setValue: (enabled) => {
                    logInfo(`${device.name} nightlight ${enabled ? 'enabled' : 'disabled'}`);
