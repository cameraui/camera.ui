...
import { firstValueFrom, fromEvent, interval, ReplaySubject, Subject, } from 'rxjs';
...
export class WebrtcConnection extends Subscribed {
    onSessionId = new ReplaySubject(1);
    onOfferSent = new ReplaySubject(1);
    dialogId = generateUuid();
    onCameraConnected = new ReplaySubject(1);
    onCallAnswered = new ReplaySubject(1);
    onCallEnded = new ReplaySubject(1);
    onError = new ReplaySubject(1);
    onMessage = new ReplaySubject();
    onWsOpen;
    onAudioRtp;
    onVideoRtp;
    pc;
    ...
    constructor(ticket, camera, options) {
        super();
        this.camera = camera;
        this.ws = new WebSocket(`wss://api.prod.signalling.ring.devices.a2z.com:443/ws?api_version=4.0&auth_type=ring_solutions&client_id=ring_site-${generateUuid()}&token=${ticket}`, {
            headers: {
    ...
    sendSessionMessage(method, body = {}) {
        ...
        if (this.sessionId) {
            ...
            this.addSubscriptions(this.onSessionId.pipe(take(1)).subscribe(sendSessionMessage));
        }
    }
    sendMessage(message) {
        if (this.hasEnded) {
    ...
    activateCameraSpeaker() {
        ...
        this.addSubscriptions(this.onCameraConnected.pipe(take(1)).subscribe(() => {
